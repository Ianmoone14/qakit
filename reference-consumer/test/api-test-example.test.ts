...
import { runApiTestExample } from '../src/run-api-test-example.js';
...
describe('reference-consumer runApiTest', () => {
  const dirs: string[] = [];
  ...
  it('returns a passed summary for GET /ping', async () => {
    const outputDir = await mkdtemp(path.join(tmpdir(), 'qakit-consumer-api-run-'));
    dirs.push(outputDir);
    const server = await startPingServer();
    try {
      const summary = await runApiTestExample({
        cwd: consumerRoot,
        outputDir,
        baseUrl: server.url,
      });
